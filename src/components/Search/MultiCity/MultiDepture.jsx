...
import {
  MultiCityStore,
  multiSearchAirPortsUpdate,
  multiSearchTextUpdate,
  updateMultiSegment,
  useMultiSearchAirports,
  useMultiSearchText,
} from "@/Store/MultiCityStore";
import { useCallback, useEffect, useMemo, useState } from "react";
import PropTypes from "prop-types";
const MultiDepture = ({ UniqueId }) => {
  ...
  //
// get the clicked departure airport and arrival airport for the heading of input
  const multiDeptureAirport = MultiCityStore.useState(
    (s) => s.Segment.find((item) => item.id === UniqueId)?.departure_airport
  );
  const multiArrivalAirport = MultiCityStore.useState(
    (s) => s.Segment.find((item) => item.id === UniqueId)?.arrival_airport
  );
  ...
  const renderAirports = useMemo(
    () =>
      multiSearchAirPorts?.length > 0
        ? multiSearchAirPorts
        : airports?.slice(0, 6),
    [multiSearchAirPorts, airports]
  );
  // Remove selected arrival airport from the list

  const finalRender = useMemo(
    () =>
      renderAirports.filter((item) => item?.code !== multiArrivalAirport?.code),
    [renderAirports, multiArrivalAirport]
  );
    ...
    const timeOut = setTimeout(() => {
      if (multi_Search_text.length > 1) {
        const newSearchAirports = airports.filter((airport) =>
          new RegExp(`\\b${multi_Search_text}`, "i").test(
  ...
  const setDepartureAirport = useCallback(
    (value) => {
      updateMultiSegment(UniqueId, "departure_airport", value);
    ...
    >
      <PopoverTrigger asChild>
        <div className="md:p-2 p-4 w-full md:w-full border bg-white/90 border-black">
          ...
          <Label
            className="text-xs md:text-[1.1vw] lg:text-xs block text-slate-900/70"
            htmlFor="email"
          >
            Form -{" "}
            {multiDeptureAirport && multiDeptureAirport?.search_contents
              ? multiDeptureAirport.search_contents
                  .split("-")
                  .slice(2, 3)
                  .join(" ")
              : ""}
          ...
          <div className="overflow-x-auto max-w-96">
            ...
            <Label
              className="text-base md:!text-[1.5vw] lg:!text-base text-nowrap"
              htmlFor="email"
            >
              {multiDeptureAirport && multiDeptureAirport?.search_contents
                ? multiDeptureAirport.search_contents
...
export default MultiDepture;

MultiDepture.propTypes = {
