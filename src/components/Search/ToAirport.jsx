...
import { ArrivalAirportUpdate, DepartureAirportUpdate, FromSearchAirportsUpdate, FromSearchTextUpdate,  SelectedAirportStore,  ToSearchAirportsUpdate,  ToSearchTextUpdate,  useArrivalAirport,  useDepartureAirport,  useFromSearchAirports,  useFromSearchText, useToSearchText } from "@/Store/SelectedAirportStore";
...
// if depture is selected in oposit then filter that 
const finalRender = renderAirports.filter((item)=> item.code !== departureAirport.code)
...
useEffect(()=>{
    ...
    const timeOut = setTimeout(()=>{
        if(to_search_text.length > 1){
            ...
            const newSearchAirports = airports.filter(airport => new RegExp(`\\b${to_search_text}`, 'i').test(airport.search_contents));
...
const handleInputChange = (e)=>{
  ...
  <div className="md:p-2 p-4   w-full md:w-full  border border-black">
    {/* render city and country  */}
            <Label className="text-xs md:text-[1.1vw] lg:text-xs  block text-slate-900/70" htmlFor="email">Form - {arrivalAirport.search_contents.split("-").slice(2,3).join(" ")}</Label>
         <div className="overflow-x-auto max-w-96">
            {/* // render code and pairport name */}
         <Label className="text-base md:!text-[1.5vw] lg:!text-base  text-nowrap " htmlFor="email">{arrivalAirport.search_contents.split("-").slice(0,2).join(" ")} </Label>
         </div>
